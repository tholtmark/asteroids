...
class Player(CircleShape):
    ...
    def update(self, dt):
        ...
        if keys[pygame.K_s]:
            self.move(-dt)
        if (
            keys[pygame.K_SPACE] and self.shot_cooldown <= 0
        ):  # checks for spacebar and cooldown
            self.shoot()
        if (
            self.shot_cooldown > 0
        ):  # this checks if cooldown still active, if so decrease it by timer
            self.shot_cooldown -= dt

    def shoot(self):
        self.shot_cooldown = (
            PLAYER_SHOOT_COOLDOWN  # sets the cooldown timer after a shot is made
        )
